<script setup lang="ts">
import { ref,  watch } from 'vue';
import { useCrudStore } from '@/stores/crud'
import { useAuthStore } from '@/stores/auth';
import { useVoiceStore } from '@/stores/voice';
import { onMounted, onUnmounted } from 'vue';

const token = useAuthStore().token;

interface Lights {
    id: number;  
    room: string;
    color: string;
    state: boolean;
}

interface Devices {
    id: number;  
    type: string;
    state: boolean;
}

const lights = ref<Array<Lights>>([]);

const devices = ref<Array<Devices>>([]);

const curtains = ref({
            id: 1,  
            openTime: 'n',
            closeTime: '',
            state: false,
        });

async function getLights() {
  await useCrudStore().getLights();
  lights.value = useCrudStore().lights;
  console.log(lights);
}

async function getDevices() {
  await useCrudStore().getDevices();
  devices.value = useCrudStore().devices;
  console.log(devices);
}

async function getCurtains() {
  await useCrudStore().getCurtains();
  curtains.value = useCrudStore().curtains;
  console.log(curtains.value);
}

function handleCheckboxClick(event: Event, type:string, id: number) {
    const checkbox = event.target as HTMLInputElement | null;
    if (checkbox) {
        console.log('Change DeviceState');
        const value = checkbox.checked;
        // Use the value and id as needed
        console.log('Checkbox ID:', id);
        console.log('Checkbox value:', value);
        useCrudStore().changeState(id, type, value);
    }
    };

    watch(() => useVoiceStore().updated, (newVal) => {
      if (newVal === 'light') {
        console.log('lights update')
        getLights();
      } else if (newVal === 'device') {
        console.log('devices update')
        getDevices();
      } else if (newVal === 'curtains') {
        console.log('curtains update')
        getCurtains();
      }
    });

    const pollingInterval = setInterval(getCurtains, 30000);

onMounted(async () => {
    getLights();
    getDevices();
    getCurtains();
});

onUnmounted(async () => {
  clearInterval(pollingInterval);
});
</script>

<template>
  <div class="bg-slate-600 rounded-3xl p-5">

    <p class="ml-6 pt-2 text-xl text-indigo-50">Lights</p>
    <div class="grid grid-cols-3 gap-20">
      <div v-if="lights != null && lights.length != 0" v-for="light in lights" :key="light.id" class="p-8 bg-indigo-200 m-5 rounded-3xl">
        <svg class="h-14 w-14 float-left text-slate-700 mr-4" aria-hidden="true" :fill="light.color" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 18v-5.25m0 0a6.01 6.01 0 001.5-.189m-1.5.189a6.01 6.01 0 01-1.5-.189m3.75 7.478a12.06 12.06 0 01-4.5 0m3.75 2.383a14.406 14.406 0 01-3 0M14.25 18v-.192c0-.983.658-1.823 1.508-2.316a7.5 7.5 0 10-7.517 0c.85.493 1.509 1.333 1.509 2.316V18" stroke-linecap="round" stroke-linejoin="round"></path>
        </svg>
        <p>Room: {{light.room}}</p>
        <label class="relative inline-flex items-center cursor-pointer mt-2">
          <input type="checkbox" :data-id="light.id" :checked="light.state" value="" @click="event => handleCheckboxClick(event, 'light', light.id)" class="sr-only peer">
          <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-indigo-600 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-400"></div>
        </label>
      </div>
    </div>
  </div>

  <div class="bg-slate-600 mt-10 rounded-3xl p-3">

    <p class="ml-6 pt-2 text-xl text-indigo-50">Devices</p>
    <div class="grid grid-cols-4 gap-8">
        <div v-if="devices != null && devices.length != 0" v-for="device in devices" :key="device.id">
            <div v-if="device.type === 'TV'" class="p-5 bg-indigo-200 m-5 rounded-3xl">
                <svg aria-hidden="true" class="h-14 w-14 float-left mr-4" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path d="M6 20.25h12m-7.5-3v3m3-3v3m-10.125-3h17.25c.621 0 1.125-.504 1.125-1.125V4.875c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125z" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg>
                <p>TV</p>
                <label class="relative inline-flex items-center cursor-pointer mt-2">
                <input type="checkbox" :checked="device.state" value="" @click="event => handleCheckboxClick(event, 'device', device.id)" class="sr-only peer">
                <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-indigo-600 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-400"></div>
                </label>
            </div>
            <div v-if="device.type === 'AC'" class="p-5 bg-indigo-200 m-5 rounded-3xl">
                <img class="h-14 w-14 float-left mr-4" src="data:image/png;base64,">
                <p>AC</p>
                <label class="relative inline-flex items-center cursor-pointer mt-2">
                <input type="checkbox" :checked="device.state" value="" @click="event => handleCheckboxClick(event, 'device', device.id)" class="sr-only peer">
                <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-indigo-600 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-400"></div>
                </label>
            </div>
            <div v-if="device.type === 'underfloor heating'" class="p-5 bg-indigo-200 m-5 rounded-3xl">
                <img class="h-14 w-14 float-left mr-4" src="data:image/png;base64,">
                <p>Floor</p>
                <label class="relative inline-flex items-center cursor-pointer mt-2">
                <input type="checkbox" :checked="device.state" value="" @click="event => handleCheckboxClick(event, 'device', device.id)" class="sr-only peer">
                <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-indigo-600 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-400"></div>
                </label>
            </div>
        </div>
      <div v-if="curtains.openTime != null && curtains.openTime != 'n'" class="p-5 bg-indigo-200 m-5 rounded-3xl">
        <img class="h-14 w-14 float-left mr-4" src="data:image/png;base64,">
        <p>Сurtains</p>
        <label class="relative inline-flex items-center cursor-pointer mt-2">
          <input type="checkbox" :checked="curtains.state" value="" @click="event => handleCheckboxClick(event, 'curtains', curtains.id)" class="sr-only peer">
          <div class="w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-blue-300 dark:peer-focus:ring-indigo-600 rounded-full peer dark:bg-gray-700 peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all dark:border-gray-600 peer-checked:bg-indigo-400"></div>
        </label>
      </div>
    </div>
  </div>
</template>