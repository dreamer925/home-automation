...
<template>
  <transition name="fade-slide" mode="out-in">
    ...
    id="toast-danger" class="flex items-center w-full max-w-xs p-4 mb-4 rounded-lg shadow text-gray-400 bg-slate-800" role="alert">
      ...
      <div class="ml-3 text-sm font-normal">{{ errorStore.error }}</div>
      <button @click="errorStore.clearError()" type="button" class="ml-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700" data-dismiss-target="#toast-danger" aria-label="Close">
